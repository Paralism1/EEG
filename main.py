...
for i in range(0, l_col_d):
    AlphaRange.append(
        np.mean(d[position_vector[0]:max(position_vector) + 1, i]))


def smoothTriangle(data, degree):
    triangle = np.concatenate(
        (np.arange(degree + 1), np.arange(degree)[::-1]))  # up then down
    smoothed = []
...
tg = np.array([
    10, 15, 20, 25, 30,35,50,55,60,70,80,90,95,100,110,120,125,135,140,150,155,160,165,175,max(t) + 1])
